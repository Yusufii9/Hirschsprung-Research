...
def copy_paste_class(dataset_class):
    def _split_transforms(self):
        ...
        if split_index is not None:
            ...
            #recreate transforms
            self.transforms = A.Compose(pre_copy, keypoint_params, additional_targets=None)
            self.post_transforms = A.Compose(post_copy, keypoint_params, additional_targets=None)
            self.copy_paste = A.Compose(
                [copy_paste], keypoint_params, additional_targets=paste_additional_targets
            )
        else:
